...
def search():
   ...
   if form.validate_on_submit():
    try:
        p=[]
        searched = form.searched.data  # Get the searched term from the form
        products = ProductClient.get_product(searched)
        url = f"http://localhost:5000/product/{searched}"  # Manually construct the URL
        return redirect(url)
    except requests.exceptions.ConnectionError:
        products = {
            'results': []
